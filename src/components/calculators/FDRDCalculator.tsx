...
const breadcrumbItems = [
  ...
  const chartData = React.useMemo(() => results ? [
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <header className="space-y-2">
          <div className="flex items-center gap-2 mb-4">
             ...
             <h1 className="text-5xl font-bold tracking-tighter text-[#f59e0b]">FD & RD Calculator</h1>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        <section className="bg-white/[0.02] border border-white/5 rounded-2xl p-8 space-y-8">
          <div className="flex gap-1 p-1 bg-white/5 rounded-xl border border-white/5">
            <button
              onClick={() => setType('FD')}
              className={`flex-1 py-3 text-base font-bold rounded-lg transition-all ${type === 'FD' ? 'bg-[#D4AF37] text-white shadow-lg shadow-[#D4AF37]/20' : 'text-white/70 hover:text-white/70'}`}
            >
              Fixed Deposit (FD)
            </button>
            <button
              onClick={() => setType('RD')}
              className={`flex-1 py-3 text-base font-bold rounded-lg transition-all ${type === 'RD' ? 'bg-[#D4AF37] text-white shadow-lg shadow-[#D4AF37]/20' : 'text-white/70 hover:text-white/70'}`}
            >
          ...
          <div className="space-y-6">
             <div className="space-y-4">
               <label className="text-base font-bold text-white/70 uppercase tracking-widest">{type === 'FD' ? 'Total Investment' : 'Monthly Deposit'}</label>
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <label className="text-base font-bold text-white/70 uppercase tracking-widest">Rate of Interest (%)</label>
                  <NumericInput 
                    step="0.1"
                    value={inputs.rate}
                    onChange={(val) => setInputs({ ...inputs, rate: val })}
                    className="w-full bg-white/5 border border-white/5 rounded-xl px-4 py-4 text-white focus:outline-none focus:ring-2 focus:ring-[#D4AF37] focus:border-[#D4AF37] transition-all font-bold"
                  />
                </div>
                <div className="space-y-4">
                  <label className="text-base font-bold text-white/70 uppercase tracking-widest">Tenure (Years)</label>
                  <NumericInput 
                    value={inputs.tenure}
               ...
               <div className="w-full md:w-1/2 h-[300px] min-h-[300px] min-w-[300px]">
                 ...
                 <div className="flex justify-center gap-6 mt-4">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-base text-white/70 font-bold uppercase tracking-widest">Invested</span>
                    </div>
                    <div className="flex items-center gap-2">
                       <div className="w-3 h-3 rounded-full bg-[#D4AF37]"></div>
                       <span className="text-base text-white/70 font-bold uppercase tracking-widest">Returns</span>
                    </div>
                 </div>
               </div>

      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <header className="space-y-2">
          <div className="flex items-center gap-2 mb-4">
             ...
             <h1 className="text-4xl font-bold tracking-tighter text-[#f59e0b]">GST / VAT Calculator</h1>
          </div>
        ...
        <section className="bg-white/[0.02] border border-white/5 rounded-2xl p-8 space-y-8">
          <div className="flex gap-1 p-1 bg-white/5 rounded-xl border border-white/5">
            <button
              ...
              className={`flex-1 py-3 text-sm font-bold rounded-lg transition-all ${inputs.isAddingTax ? 'bg-[#D4AF37] text-white shadow-lg shadow-[#D4AF37]/20' : 'text-white/70 hover:text-white/70'}`}
            >
              Add GST
            </button>
            <button
          ...
          <div className="space-y-6">
             <div className="space-y-4">
               <label className="text-sm font-bold text-white/70 uppercase tracking-widest">Amount</label>
             ...
             <div className="space-y-4">
                <label className="text-sm font-bold text-white/70 uppercase tracking-widest">Tax Slab (%)</label>
                <div className="grid grid-cols-4 gap-2 text-center">
                  {slabs.map(slab => (
                    <button
                      key={slab}
                      onClick={() => setInputs({ ...inputs, taxRate: slab })}
                      className={`py-3 rounded-xl border transition-all text-sm font-bold ${inputs.taxRate === slab ? 'bg-[#D4AF37]/20 border-[#D4AF37] text-white' : 'bg-white/5 border-white/5 text-white/70'}`}
                    >
                      {slab}%
                    </button>
                  ))}
                  <div className="col-span-4 mt-2">
                     <NumericInput 
                       ...
                       className="w-full bg-white/5 border border-white/5 rounded-xl px-4 py-2 text-sm text-white focus:outline-none focus:border-[#D4AF37]"
                     />
                  </div>
                </div>
             </div>
           ...
           {isMounted && (
             <div className="w-full space-y-6">
                <div className="p-6 bg-white/5 rounded-2xl border border-white/5 flex items-center justify-between">
                   <div>
                     <div className="text-sm font-bold text-white/70 uppercase tracking-widest mb-1">Tax Amount</div>
                     <div className="text-4xl font-bold text-[#D4AF37] tracking-tighter">{formatCurrency(results.taxAmount)}</div>
                   </div>
                ...
                <div className="p-6 bg-white/[0.02] rounded-2xl border border-white/5 flex items-center justify-between">
                   <div>
                     <div className="text-sm font-bold text-white/70 uppercase tracking-widest mb-1">{inputs.isAddingTax ? 'Net Price (Before Tax)' : 'Base Price'}</div>
                     <div className="text-2xl font-bold text-white/70 tracking-tighter">{formatCurrency(results.originalAmount)}</div>
                   </div>
                </div>

                <div className="p-8 bg-[#D4AF37]/10 rounded-2xl border border-[#D4AF37]/20 shadow-xl shadow-[#D4AF37]/5">
                   <div className="text-sm font-bold text-[#D4AF37] uppercase tracking-widest mb-1">{inputs.isAddingTax ? 'Total Billing Value' : 'Net Price'}</div>
                   <div className="text-5xl font-bold tracking-tighter text-white">{formatCurrency(results.totalAmount)}</div>
                </div>
             </div>
           )}
        </section>

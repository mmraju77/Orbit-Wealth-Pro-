      ...
      <table className="w-full text-left">
        <thead>
          <tr className="text-sm uppercase tracking-widest text-white/70 border-b border-white/5">
            ...
            <th className="py-3 font-medium text-right text-white/70">Total Interest</th>
        ...
        <tbody className="text-[13px] font-light tabular-nums divide-y divide-white/5">
          {schedule.filter((_, i) => i % 12 === 0 || i === schedule.length - 1).map((period, i) => (
            ...
            >
              <td className="py-3 text-white/70">Year {Math.floor(period.period / 12) + 1}</td>
              ...
              <td className="py-3 text-right text-white/70">{formatCurrency(period.totalInterestPaid)}</td>

  ...
  return (
    <div className="mt-20 space-y-12 border-t border-white/5 pt-20 pb-20">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <ul className="space-y-4">
            {howTo.map((step, idx) => (
              <li key={idx} className="flex gap-4 text-sm text-white/40 leading-relaxed">
                <span className="text-[#0055FF] font-bold">0{idx + 1}.</span>
                {step}
              </li>
            ))}
          </ul>
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 mb-4">
            <FileText className="text-[#0055FF] w-5 h-5" />
          ...
          <div className="p-6 bg-white/[0.02] border border-white/5 rounded-xl font-mono text-xs text-[#0055FF] leading-relaxed overflow-x-auto whitespace-nowrap">
            {formula}
          </div>
      ...
      <div className="bg-[#0055FF]/5 border border-[#0055FF]/10 rounded-2xl p-8">
        <div className="flex items-center gap-2 mb-6">
          <ListChecks className="text-[#0055FF] w-5 h-5" />
          <h3 className="text-xl font-bold text-white tracking-tight">Key Benefits</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {benefits.map((benefit, idx) => (
            <div key={idx} className="flex items-start gap-3">
              <div className="w-1.5 h-1.5 rounded-full bg-[#0055FF] mt-2 shrink-0"></div>

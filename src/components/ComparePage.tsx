...
const REGIONS: { id: Region; name: string; icon: React.ReactNode }[] = [
  ...
  { id: 'India', name: 'India', icon: <span className="text-[10px] font-bold">IN</span> },
  { id: 'USA', name: 'USA', icon: <span className="text-[10px] font-bold">US</span> },
  { id: 'UK', name: 'UK', icon: <span className="text-[10px] font-bold">UK</span> },
];
    ...
    <div className="space-y-12 pb-20">
      ...
      <header className="space-y-6 text-center max-w-3xl mx-auto">
        <motion.div 
          ...
          className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-emerald-500/5 mb-4"
        >
          <Zap className="w-3 h-3" /> AI-Augmented Comparison Framework
        </motion.div>
        <h1 className="text-4xl md:text-6xl font-display font-bold tracking-tighter text-white leading-[0.9]">
      ...
      <section className="bg-white/[0.02] border border-white/5 rounded-[3rem] overflow-hidden shadow-2xl relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 bg-white/[0.03] border-b border-white/5">
          ...
          <div className="p-8 text-2xl font-display font-bold text-[#D4AF37] flex items-center border-t md:border-t-0 border-white/5">
             <div className="w-8 h-8 rounded-full bg-[#D4AF37]/10 flex items-center justify-center text-[10px] mr-3 border border-[#D4AF37]/20">01</div>
             {data.optionAName}
          </div>
          <div className="p-8 text-2xl font-display font-bold text-emerald-400 flex items-center border-t md:border-t-0 border-white/5">
             <div className="w-8 h-8 rounded-full bg-emerald-500/10 flex items-center justify-center text-[10px] mr-3 border border-emerald-500/20">02</div>
             {data.optionBName}
          </div>
        </div>
        ...
        {data.factors.map((f, idx) => (
          ...
          >
            ...
            <div className="p-8 text-sm text-slate-100 border-t md:border-t-0 border-white/5 flex items-center font-bold tracking-tight">
                {f.optionA}
            </div>
            <div className="p-8 text-sm text-slate-100 border-t md:border-t-0 border-white/5 flex items-center font-bold tracking-tight bg-emerald-500/[0.01]">
                {f.optionB}
            </div>
          </motion.div>
        ))}
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="p-8 bg-white/[0.01] border border-white/5 rounded-3xl space-y-4">
             <div className="flex justify-between text-[10px] font-black uppercase tracking-[0.2em] text-[#D4AF37]">
          ...
          <div className="space-y-10">
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <h4 className="text-[11px] font-black text-emerald-400 uppercase tracking-widest">Growth Catalysts</h4>
              </div>
              <ul className="grid grid-cols-1 gap-4">
                {data.prosA.map((p, i) => (
                  <li key={i} className="flex items-start gap-4 p-4 bg-white/[0.01] border border-white/5 rounded-2xl group/item hover:bg-white/[0.03] transition-colors">
                    ...
                    <span className="text-sm text-slate-300 leading-snug font-medium">{p}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="space-y-5 pt-8 border-t border-white/5">
              <div className="flex items-center gap-3">
                ...
                <h4 className="text-[11px] font-black text-rose-400 uppercase tracking-widest">Strategic Risks</h4>
              </div>
              <ul className="grid grid-cols-1 gap-4">
                {data.consA.map((c, i) => (
                  <li key={i} className="flex items-start gap-4 p-4 bg-white/[0.01] border border-white/5 rounded-2xl opacity-60 group/item hover:opacity-100 transition-opacity">
                    ...
                    <span className="text-sm text-slate-400 leading-snug font-medium italic">{c}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="space-y-10">
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <h4 className="text-[11px] font-black text-emerald-400 uppercase tracking-widest">Preservation Factors</h4>
              </div>
              <ul className="grid grid-cols-1 gap-4">
                {data.prosB.map((p, i) => (
                  <li key={i} className="flex items-start gap-4 p-4 bg-white/[0.01] border border-white/5 rounded-2xl group/item hover:bg-white/[0.03] transition-colors">
            ...
            <div className="space-y-5 pt-8 border-t border-white/5">
              <div className="flex items-center gap-3">
                ...
                <h4 className="text-[11px] font-black text-rose-400 uppercase tracking-widest">Market Exposure</h4>
              </div>
              <ul className="grid grid-cols-1 gap-4">
                {data.consB.map((c, i) => (
                  <li key={i} className="flex items-start gap-4 p-4 bg-white/[0.01] border border-white/5 rounded-2xl opacity-60 group/item hover:opacity-100 transition-opacity">
      ...
      <section className="bg-white/[0.02] border border-white/10 rounded-[3rem] p-10 md:p-16 relative overflow-hidden group shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)]">
        ...
        <div className="relative z-10 space-y-12">
          ...
          <div className="p-10 bg-white/[0.03] border-l-4 border-[#D4AF37] rounded-3xl relative backdrop-blur-sm">
            <p className="text-3xl text-slate-100 leading-snug font-bold tracking-tight italic">
                "{data.verdict}"
            </p>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-6 pt-6">
              <Link 
                  to={data.ctaLink}
                  className="w-full md:w-auto inline-flex items-center justify-center gap-5 px-12 py-6 bg-[#D4AF37] hover:bg-[#f59e0b] text-[#0B1221] rounded-2xl font-black text-xs uppercase tracking-[0.3em] transition-all shadow-[0_25px_60px_-10px_rgba(212,175,55,0.3)] hover:scale-105 active:scale-95 group"
              >
                  {data.ctaText}
              ...
              <Link 
                  to="/"
                  className="w-full md:w-auto inline-flex items-center justify-center gap-5 px-12 py-6 bg-white/5 hover:bg-white/10 text-white rounded-2xl font-black text-xs uppercase tracking-[0.3em] transition-all border border-white/10 backdrop-blur-xl"
              >
